...
protected:
 ...
 void OnWrite(uint16_t idx, uint8_t subidx) noexcept override
  { if (idx == 0x3366 && subidx == 0)
    ...
    if (!message_thread.joinable())
    {
      message_thread = std::thread(std::bind(&FakeMotorController::fake_periodic_messages, this));
    }
  }
...
protected:
  ...
  void run() override
  {
    ...
    // timer.submit_wait()
    RCLCPP_INFO(this->get_logger(), "Created slave for node_id %i.", static_cast<uint8_t>(node_id_-12));
    loop.run();
    ctx.shutdown();
